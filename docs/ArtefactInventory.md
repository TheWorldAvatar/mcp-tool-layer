...
- **Generated by**:
  ...
  - Optional standalone helper: `python -m src.agents.scripts_and_prompts_generation.iteration_creation_agent ...`
- **Consumed by**:
...
- **Candidate (generated)**:
  - `ai_generated_contents_candidate/scripts/<ontology>/<ontology>_creation_base.py` (+ checks / entities / relationships modules as emitted by the deterministic generator)
...
- **Generated by**:
  - `python -m src.agents.scripts_and_prompts_generation.agentic_generation_main ...`
  - Optional Docker/agent flows may still call `python -m src.agents.scripts_and_prompts_generation.mcp_main_script_creation_agent ...`
...
Generation pipeline extras:
- **`configs/generated_ontology_mcps.json`** may appear on older branches that still ran the retired `generation_main.py` orchestrator. Fresh regenerations should rely on `agentic_generation_main.py` plus `configs/run_created_mcp.json` / `configs/extension.json`.
