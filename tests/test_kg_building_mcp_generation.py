...
from src.extraction_prompt_generation.runtime_support import om2 as om2_runtime
from src.kg_building_mcp_generation.surface.infer import (
...
def test_generated_output_uses_graph_idempotency_and_fresh_identity() -> None:
    ...
    assert "ensure=_ensure_missing_owner_facets" not in operations
...
def test_declared_bridge_class_makes_representation_label_required() -> None:
    ...
    assert "The MCP never reads the ledger." in compiled["instruction"]
    assert "Required representation labels compiled from declared extension bridge" not in (
    ...
    assert "Required representation labels:" not in output_body
...
def test_generator_modules_contain_no_ontology_literals() -> None:
    root = Path("src/kg_building_mcp_generation")
    forbidden = (
        "OntoSyn",
        "OntoSynthesis",
        "ontosyn:",
...
def test_example_fixture_emits_persistent_replay_and_facet_receipts() -> None:
    ...
    assert 'payload["skippable"] = False' in operations
    assert 'payload["source_value"] = str(label)' in operations
    assert "QUANTITY_UNIT_CLASS_MISMATCH" in operations
    assert "register_semantic_rejection(" in operations
...
def test_generated_example_runtime_replays_and_rejects_bad_facet() -> None:
    class_iri = NS + "Record"
    ...
    namespace = {
        ...
        "om2_runtime": om2_runtime,
    }
    exec(compile(tree, "<generated-occurrence>", "exec"), namespace)

    rejected = json.loads(namespace["create_Record"]("sample", hasMeasure="bad"))
    assert rejected["status"] == "rejected"
    assert rejected.get("skippable") is False
    assert list(graph) == []

    created = json.loads(namespace["create_Record"]("sample"))
    assert created["status"] == "ok"
    assert created["graph_changed"] is True
    assert created["already_committed"] is False
    assert created["graph_revision"] == 1
    assert created.get("omitted_facet") is False
    assert (URIRef(created["iri"]), RDF.type, URIRef(class_iri)) in graph

    replayed = json.loads(namespace["create_Record"](" SAMPLE "))
    ...
    after_restart = json.loads(restarted_namespace["create_Record"]("sample"))
...
def _extension_surface_fixture(*, focus_reusable: bool) -> tuple[dict, dict]:
    parsed, contract = _fixture()
    parsed["classes"]["Focus"] = {
        "iri": NS + "Focus",
        "parent_classes": [],
        "comment": "Adopted extension focus seeded from the parent graph.",
    }
    parsed["classes"]["Part"] = {
        "iri": NS + "Part",
        "parent_classes": [],
        "comment": "Reusable part that hangs off the adopted focus.",
    }
    parsed["properties"]["hasPart"] = {
        "comment": "A focus owns one or more parts."
    }
    parsed["properties"]["focusCode"] = {"comment": "Optional code on the focus."}
    parsed["properties"]["partCode"] = {"comment": "Optional code on a part."}
    publish = contract["ontology_publish_contract"]
    publish["subclass_closure"].extend(
        [
            {"class_iri": NS + "Focus", "superclass_iris": []},
            {"class_iri": NS + "Part", "superclass_iris": []},
        ]
    )
    publish["datatype_properties"].extend(
        [
            {
                "property_iri": NS + "focusCode",
                "domain_iris": [NS + "Focus"],
                "range_iris": [XSD + "string"],
            },
            {
                "property_iri": NS + "partCode",
                "domain_iris": [NS + "Part"],
                "range_iris": [XSD + "string"],
            },
        ]
    )
    contract["relationship_tool_contracts"]["hasPart"] = {
        "predicate_local": "hasPart",
        "predicate_iri": NS + "hasPart",
        "domain_iris": [NS + "Focus"],
        "range_iris": [NS + "Part"],
    }
    contract["reuse_policy"]["classes"].extend(
        [
            {
                "class_iri": NS + "Focus",
                "class_local": "Focus",
                "reusable": focus_reusable,
                "reuse_scope": "global" if focus_reusable else "occurrence_local",
            },
            {
                "class_iri": NS + "Part",
                "class_local": "Part",
    ...
    contract["extension_focus"] = {
        "class_local": "Focus",
        "class_iri": NS + "Focus",
    }
    contract["extension_bridge_class_iris"] = [NS + "Focus"]
    return parsed, contract


def test_extension_reusable_focus_keeps_public_child_create() -> None:
    parsed, contract = _extension_surface_fixture(focus_reusable=True)
    candidates = discover_occurrence_surface_candidates(
        parsed=parsed, contract=contract
    )
    parent_links = [
        item for item in candidates["candidates"] if item["kind"] == "parent_link"
    ]
    assert any(item["owner_class_local"] == "Part" for item in parent_links)
    assert not any(item["owner_class_local"] == "Focus" for item in parent_links)
    nested_to_part = [
        item
        for item in candidates["candidates"]
        if item.get("target_class_local") == "Part"
        and item["kind"] in {"reusable_link", "fresh_dependent"}
    ]
    assert nested_to_part == []
    contract["occurrence_surface_candidates"] = candidates
    contract["occurrence_surface_decisions"] = _bundle_all(candidates)
    compiled = compile_occurrence_surface(parsed=parsed, contract=contract)
    assert compiled["errors"] == []
    ...
    focus = tools["create_Focus"]
    part = tools["create_Part"]
    assert focus["adopted_focus"] is True
    assert focus["identity_contract"] == {"kind": "adopted_focus", "identity_args": []}
    assert not focus["parent_parameter"]
    assert part["parent_parameter"] == "parent_iri"
    assert part["parent_predicate_local"] == "hasPart"
    assert part["parent_binds_to_session_root"] is False
    assert part["identity_contract"] == {
        "kind": "semantic_occurrence",
        "identity_args": ["parent_iri", "label"],
    }
    assert "pipeline-seeded enrichment IRI" in compiled["instruction"]
    assert "do not substitute the bound root" in compiled["instruction"]
    context = SimpleNamespace(
        ontology=SimpleNamespace(name="example"),
        contract={"occurrence_surface_units": compiled},
    ...
    operations = emit_occurrence_operations(context, compiled)
    focus_body = operations.split("def create_Focus(", 1)[1].split("\ndef ", 1)[0]
    part_body = operations.split("def create_Part(", 1)[1].split("\ndef ", 1)[0]
    assert "bound_enrichment_target_iri" in focus_body
    assert "ensure=_ensure_missing_owner_facets" not in focus_body
    assert "bind_parent_occurrence_argument(parent_iri)" in part_body
    assert "bind_root_argument(parent_iri)" not in part_body


def test_extension_live_focus_keeps_reusable_child_nested() -> None:
    parsed, contract = _extension_surface_fixture(focus_reusable=False)
    candidates = discover_occurrence_surface_candidates(
        parsed=parsed, contract=contract
    )
    contract["occurrence_surface_candidates"] = candidates
    ...
    tools = {item["name"]: item for item in compiled["public_tools"]}
    assert "create_Focus" in tools
    assert "create_Part" not in tools
    assert tools["create_Focus"]["identity_contract"]["kind"] == "adopted_focus"
    assert any(
        item["label_parameter"] == "hasPart_label"
        for item in tools["create_Focus"]["reusable_links"]
    )
    context = SimpleNamespace(
        ontology=SimpleNamespace(name="example"),
        contract={"occurrence_surface_units": compiled},
    ...
    operations = emit_occurrence_operations(context, compiled)
    focus_body = operations.split("def create_Focus(", 1)[1].split("\ndef ", 1)[0]
    assert "ensure=_ensure_missing_owner_facets" in focus_body
    assert "hasPart_label = hasPart_label or _optional_label(label)" not in focus_body
...
def _adopted_focus_identifier_tool(*, identifier_iri: str) -> dict:
    return {
        "name": "create_Focus",
        "primitive_tool": "create_Focus",
        "owner_class_iri": NS + "Focus",
        "ordering_property_local": "",
        "parent_parameter": "",
        ...
        "reusable_links": [
            {
                "label_parameter": "hasIdentifier_label",
                "predicate_local": "hasIdentifier",
                "predicate_iri": NS + "hasIdentifier",
                "target_class_local": "Identifier",
                "target_class_iri": identifier_iri,
                "create_tool": "create_Identifier",
                "create_fresh_with_datatypes": True,
                "datatype_inputs": [
                    {
                        "property_local": "hasCode",
                        "parameter_name": "hasIdentifier_hasCode",
                    }
                ],
            }
        ],
...
def _adopted_focus_runtime(graph: Graph, *, focus_iri: str):
    def success_json(*, iri: str = "", message: str = "", **metadata: object) -> str:
        return json.dumps(
            {"status": "ok", "iri": iri, "message": message, **metadata},
            sort_keys=True,
    ...
    return (
        SimpleNamespace(
            ...
            bound_enrichment_target_iri=lambda _class_iri: focus_iri,
            register_semantic_rejection=lambda *_args, **_kwargs: None,
            resolve_semantic_skip=lambda *_args, **_kwargs: "{}",
        ),
        success_json,
...
def test_adopted_focus_reuse_fills_missing_reusable_link() -> None:
    focus_iri = "urn:test:focus"
    identifier_iri = NS + "Identifier"
    created_id = "urn:test:identifier"
    compiled = {
        "public_tools": [_adopted_focus_identifier_tool(identifier_iri=identifier_iri)],
        "public_linkers": [],
        "reusable_classes": [],
    }
    graph = Graph()
    graph.add((URIRef(focus_iri), RDF.type, URIRef(NS + "Focus")))
    graph.add((URIRef(focus_iri), RDFS.label, Literal("focus-1")))
    runtime, success_json = _adopted_focus_runtime(graph, focus_iri=focus_iri)
    created_labels: list[str] = []
    created_codes: list[object] = []

    def create_identifier(*, label, hasCode=None):
        created_labels.append(label)
        created_codes.append(hasCode)
        graph.add((URIRef(created_id), RDF.type, URIRef(identifier_iri)))
        graph.add((URIRef(created_id), RDFS.label, Literal(label)))
        if hasCode is not None:
            graph.add((URIRef(created_id), URIRef(NS + "hasCode"), Literal(hasCode)))
        return success_json(iri=created_id)

    def add_has_identifier(subject_iri, object_iri):
        graph.add((URIRef(subject_iri), URIRef(NS + "hasIdentifier"), URIRef(object_iri)))
        return success_json(iri=object_iri)
    ...
    operations = emit_occurrence_operations(context, compiled)
    assert "ensure=_ensure_missing_owner_facets" in operations
    assert "hasIdentifier_label = hasIdentifier_label or _optional_label(label)" not in operations
    ...
    namespace = {
        "rdf_runtime": runtime,
        "entities": SimpleNamespace(
            create_Focus=lambda **_kwargs: success_json(iri="urn:test:fresh"),
            create_Identifier=create_identifier,
        ),
        "relationships": SimpleNamespace(add_hasIdentifier=add_has_identifier),
    }
    exec(compile(tree, "<adopted-focus-reuse-identifier>", "exec"), namespace)
    reused = json.loads(
        namespace["create_Focus"](
            "focus-1", hasIdentifier_label="id-1", hasIdentifier_hasCode="code-1"
        )
    )
    assert reused["iri"] == focus_iri
    assert reused["already_committed"] is True
    assert reused["graph_changed"] is True
    assert created_labels == ["id-1"]
    assert created_codes == ["code-1"]
    assert (
        URIRef(focus_iri),
        URIRef(NS + "hasIdentifier"),
        URIRef(created_id),
    ) in graph
    replayed = json.loads(
        namespace["create_Focus"](
    ...
    assert replayed["iri"] == focus_iri
    assert replayed["graph_changed"] is False
    assert created_labels == ["id-1"]


def test_adopted_focus_reuse_skips_empty_optional_identifier() -> None:
    focus_iri = "urn:test:focus"
    identifier_iri = NS + "Identifier"
    compiled = {
        "public_tools": [_adopted_focus_identifier_tool(identifier_iri=identifier_iri)],
    ...
    def create_identifier(*, label, hasCode=None):
        created_labels.append(label)
        return success_json(iri="urn:test:identifier")
    ...
    namespace = {
        ...
        "relationships": SimpleNamespace(
            add_hasIdentifier=lambda *_args: success_json(iri=focus_iri)
        ),
    }
    exec(compile(tree, "<adopted-focus-skip-identifier>", "exec"), namespace)
    absent = json.loads(namespace["create_Focus"]("focus-1"))
    assert absent["iri"] == focus_iri
    sentinel = json.loads(
        namespace["create_Focus"]("focus-1", hasIdentifier_label="n/a")
    )
    assert sentinel["iri"] == focus_iri
    assert created_labels == []
    assert list(graph.objects(URIRef(focus_iri), URIRef(NS + "hasIdentifier"))) == []


def test_cli_keeps_only_main_settings() -> None:
    from src.kg_building_mcp_generation.cli import FOUR_DOMAINS, build_parser
    ...
    assert FOUR_DOMAINS == ("ontosynthesis", "ontomops", "ontospecies", "medical")


def test_precheck_requires_existing_domain_inputs() -> None:
    from pathlib import Path
    ...
    from src.kg_building_mcp_generation.cli import precheck_domain

    repo = repository_root()
    precheck_domain(
        ontology="ontosynthesis",
        domain_config=resolve_domain_config_path(ontology_name="ontosynthesis"),
        output_root=repo / "generated",
        repository_root=repo,
        require_upstream=False,
    )
    missing = Path(repo / "generated" / "_missing_parent_for_precheck")
    try:
        precheck_domain(
            ontology="ontomops",
            domain_config=resolve_domain_config_path(ontology_name="ontomops"),
            output_root=missing,
            repository_root=repo,
            require_upstream=True,
        )
    except FileNotFoundError as exc:
        assert "ontosynthesis" in str(exc)
    else:
        raise AssertionError("extension precheck must require the parent context")
