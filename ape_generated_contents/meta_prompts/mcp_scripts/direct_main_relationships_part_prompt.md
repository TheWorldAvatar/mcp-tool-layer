...
4) Preserve the exact parameter annotations (including `Annotated[..., Field(...)]`) and concise docstrings from the underlying definitions. If a wrapped signature uses Annotated/Field, import them (`from typing import Annotated`; `from pydantic import Field`).
5) Do not start server on import: only call `mcp.run(...)` under `if __name__ == "__main__":`.
