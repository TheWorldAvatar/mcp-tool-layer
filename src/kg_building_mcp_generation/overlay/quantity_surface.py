...
def _aliases_by_class(unit_local_to_class: dict[str, str]) -> dict[str, list[str]]:
    ...
    for alias, unit_iri in OM2_UNIT_MAP.items():
        class_iri = unit_local_to_class.get(_local(str(unit_iri)))
        if not class_iri:
            continue
        aliases.setdefault(class_iri, []).append(str(alias))
    for class_iri, values in aliases.items():
        aliases[class_iri] = sorted(dict.fromkeys(values), key=lambda item: (len(item), item))
    return aliases
