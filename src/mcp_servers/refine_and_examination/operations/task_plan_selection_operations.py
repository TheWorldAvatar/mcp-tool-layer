...
def output_selected_task_index(meta_task_name: str, selected_task_index: List[int]) -> str:
    # write the selected task index to a file
    
    # check whether the selected task index is valid and == 2 
    if len(selected_task_index) != 2:
        return f"Error: Selected task index must be a list of length 2. The selected task index is {selected_task_index}"

    # check the selected task index is in the range of 0 to 2, this is not a sorted list
    for index in selected_task_index:
        if index < 0 or index > 2:
            return f"Error: Selected task index must be in the range of 0 to 2. The selected task index is {selected_task_index}"

    selected_task_index_path = f"file://{os.path.join(SANDBOX_TASK_DIR, meta_task_name, 'selected_task_index.json')}"
    write_result = safe_write_json(selected_task_index_path, selected_task_index)
    return f"Selected task index {selected_task_index} output status: {write_result}" 