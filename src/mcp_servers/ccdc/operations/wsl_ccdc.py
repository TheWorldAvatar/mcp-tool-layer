...
def _subprocess_env() -> dict:
    ...
    CCDC MCP may be launched with a stripped env. Licensed CSD still needs Windows
    and conda variables or the child hangs until the subprocess timeout.
    """
    root = _project_root()
    out = {**os.environ}
    ...
    if os.name == "nt":
        windir = out.get("SYSTEMROOT") or out.get("WINDIR") or r"C:\Windows"
        out.setdefault("SYSTEMROOT", windir)
        out.setdefault("WINDIR", windir)
        out.setdefault("SYSTEMDRIVE", "C:")
        out.setdefault("COMSPEC", str(Path(windir) / "System32" / "cmd.exe"))
        csd_py = (out.get("CSD_PYTHON_EXE") or "").strip()
        path_parts = [item for item in (out.get("PATH") or "").split(os.pathsep) if item]
        if csd_py:
            csd_bin = str(Path(csd_py).resolve().parent)
            if csd_bin not in path_parts:
                path_parts.insert(0, csd_bin)
        system32 = str(Path(windir) / "System32")
        if system32 not in path_parts:
            path_parts.append(system32)
...
def _ccdc_lock_wait_sec() -> float:
    """How long to wait for the cross-process CSD lock before fail-closed."""
    raw = (os.environ.get("CCDC_CSD_LOCK_WAIT_SEC") or "600").strip()
    try:
        return max(30.0, float(raw))
    except Exception:
        return 600.0
...
def _csd_lock_path() -> Path:
    override = (os.environ.get("CCDC_CSD_LOCK_FILE") or "").strip()
    if override:
        return Path(override)
    try:
        return Path(DATA_CCDC_DIR) / "mcp_ccdc_csd.lock"
    except Exception:
        pass
    base = os.environ.get("LOCALAPPDATA") or os.environ.get("USERPROFILE") or str(Path.home())
    return Path(base) / "mcp_ccdc_csd.lock"


@contextmanager
def _exclusive_csd_lock() -> Iterator[None]:
    """Serialize licensed-CSD subprocesses. The CSD license is not parallel-safe."""
    path = _csd_lock_path()
    path.parent.mkdir(parents=True, exist_ok=True)
    deadline = time.monotonic() + _ccdc_lock_wait_sec()
    fh = open(path, "a+b")
    if fh.tell() == 0:
        fh.write(b"\0")
        fh.flush()
            ...
            while True:
                try:
                    ...
                    if time.monotonic() >= deadline:
                        raise TimeoutError(f"timed out waiting for CSD lock {path}")
                    time.sleep(0.25)
        else:
            import fcntl
...
def _lookup_hardcoded_ccdc(name: str) -> List[Tuple[str, str]]:
    ...
    if raw:
        ...
        raw_lower = raw.lower()
        for haystack in (destemmed, raw_lower):
            token = re.search(r"\b([a-z][a-z0-9]*-\d+)\b", haystack)
...
def _live_csd_json_array(args: list[str]) -> Optional[list]:
    """Run licensed CSD once; return a JSON array or None on timeout/error."""
    code, out, err = _run_csd_windows_ccdc_safe(args)
    if code != 0 or not (out or "").strip():
        _diag(
            f"[CCDC] live CSD failed code={code} "
            f"args={args[:6]!r} stderr={(err or '')[:400]!r}"
        )
        return None
    try:
        parsed = _parse_json_array_text(out)
    except Exception as exc:
        _diag(f"[CCDC] live CSD JSON parse error: {exc}; stderr={(err or '')[:400]!r}")
        return None
...
def _live_search_ccdc_by_exact_name(name: str) -> List[Tuple[str, str]]:
    """Exact compound-name search against licensed CSD. Anywhere mode stays off."""
    query = (name or "").strip()
    if not query:
        return []
    rows = _live_csd_json_array(["search", "--name", query, "--exact"])
    if not rows:
        return []
    ...
    for item in rows:
        if not isinstance(item, (list, tuple)) or len(item) < 2:
            continue
        refcode = str(item[0] or "").strip()
        number = str(item[1] or "").strip()
        if refcode and number:
            hits.append((refcode, number))
    return hits


def search_ccdc_by_mop_name(name: str, exact: bool = True) -> List[Tuple[str, str]]:
    """Resolve a compound name via curated mapping, then one licensed-CSD exact search.

    The ``exact`` flag is ignored for live CSD: free-text ``anywhere`` searches stay
    disabled. Timeouts and empty CSD hits fail closed so callers can try DOI once.
    """
    del exact
    ...
    live_results = _live_search_ccdc_by_exact_name(query_name or name)
    if live_results:
        _diag(f"[CCDC] Live CSD exact-name hit for '{name}': {live_results}")
        return live_results
    _diag(f"[CCDC] No exact-name mapping or live CSD hit for '{name}'; returning no result")
    return []
...
def search_ccdc_by_doi(doi_like: str) -> List[Dict[str, str]]:
    """Look up CCDC numbers for a DOI via licensed CSD. Empty/timeout fail closed.

    ``main.search_ccdc_by_doi`` still returns curated DOI rows first. This path
    runs only when that table has no key for the DOI.
    """
    doi = _normalize_doi_input(doi_like)
    rows = _live_csd_json_array(["search_doi", "--doi", doi])
    if not rows:
        _diag(f"[CCDC] DOI '{doi}' live CSD returned no result")
        return []
    cleaned: List[Dict[str, str]] = []
    for row in rows:
        if not isinstance(row, dict):
            continue
        number = str(row.get("ccdc_number") or "").strip()
        if not number:
            continue
        cleaned.append(
            {
                "refcode": str(row.get("refcode") or ""),
                "chemical_name": str(row.get("chemical_name") or ""),
                "formula": str(row.get("formula") or ""),
                "ccdc_number": number,
                "doi": str(row.get("doi") or doi),
            }
        )
    if cleaned:
        _diag(f"[CCDC] Live CSD DOI hit for '{doi}': {len(cleaned)} row(s)")
    else:
        _diag(f"[CCDC] DOI '{doi}' live CSD returned no usable CCDC numbers")
    return cleaned
