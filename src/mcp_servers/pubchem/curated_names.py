...
def curated_name_records(name: str) -> list[dict[str, Any]] | None:
    """Curated hit as a FastMCP list, or None to fall through to NCBI."""
    record = curated_pubchem_record(name)
    if record is None:
        return None
    from src.mcp_servers.pubchem.name_dedup import finalize_pubchem_list

    return finalize_pubchem_list([record], query=name, use_llm=False)
