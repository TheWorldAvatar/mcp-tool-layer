...
ontosyn:NonVacuousGraphShape a sh:NodeShape ;
  sh:targetNode ontosyn:GraphIntegrityAnchor ;
  sh:sparql [
    sh:message "Graph must contain at least one ontosyn:ChemicalSynthesis" ;
    sh:select '''
      PREFIX ontosyn: <https://www.theworldavatar.com/kg/OntoSyn/>
      SELECT $this
      WHERE {
        FILTER NOT EXISTS { ?cs a ontosyn:ChemicalSynthesis }
      }
    '''
  ] .
...
ontosyn:ForbiddenCrystallizeShape a sh:NodeShape ;
  sh:targetClass ontosyn:Crystallize ;
  sh:sparql [
    sh:message "ontosyn:Crystallize is not in the T-Box. Use HeatChill for a crystal-yielding hold." ;
    sh:select '''
      PREFIX ontosyn: <https://www.theworldavatar.com/kg/OntoSyn/>
      SELECT $this
      WHERE {
        $this a ontosyn:Crystallize .
      }
    '''
  ] .
