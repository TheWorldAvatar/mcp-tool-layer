...
def _is_absolute_iri(node_id: str) -> bool:
    text = str(node_id or "")
    return text.startswith("http://") or text.startswith("https://")


def scope_token(scope: str) -> str:
    """Sanitize a bound-root URI or label into a TTL-safe occurrence prefix."""
...
def _occurrence_local_name(node_id: str) -> str:
...
def _is_bound_chemical_synthesis(node: Node) -> bool:
    return _norm_type(node) == "chemicalsynthesis"


def scope_occurrence_ids(graph: GraphDocument | None, scope: str) -> GraphDocument | None:
    """Prefix occurrence-local ids so per-entity TTL IRIs cannot collide.

    Reusable inventory individuals and the bound ChemicalSynthesis keep their
    ids. Owner / non-reusable nodes are prefixed even when the current id is
    already an absolute IRI — convert unions ``ontosynthesis_output/*.ttl`` by
    IRI, so two ``ChemicalOutput-1`` nodes would otherwise become one product.
    """
    if graph is None:
        return None
    ...
    for node in graph.nodes:
        current = str(node.id or "")
        if is_cross_entity_reusable(node) or _is_bound_chemical_synthesis(node):
            id_map[node.id] = current
            continue
        local = _occurrence_local_name(current)
        if local.startswith(prefix):
            id_map[node.id] = local
        else:
            id_map[node.id] = f"{prefix}{local}"
    nodes = [copy_node(node, node_id=id_map[node.id]) for node in graph.nodes]
    by_id = {node.id: node for node in nodes}
    relationships = []
    for rel in graph.relationships:
...
def is_cross_document_reusable(node: Node) -> bool:
    local = _norm_type(node)
    if local == "metalorganicpolyhedron":
        # Same-paper MOP reuse stays in is_cross_entity_reusable. A CCDC-bearing
        # MOP must not seed the next paper's CROSS_DOCUMENT inventory.
        return False
...
def identity_key(node: Node, *, reuse: str = "layer") -> tuple:
    ...
    if reuse == "paper":
        if not is_cross_entity_reusable(node):
            # Occurrence-local ids such as ChemicalOutput-1 collide across
            # syntheses. Do not use them as a match key; attach_subgraph remaps.
            return ("fresh", id(node))
