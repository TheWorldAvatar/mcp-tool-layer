...
def curie(iri: str) -> str:
    ...
    if iri.startswith(MEDICAL):
        return f"medical:{iri[len(MEDICAL):]}"
...
def load_ontology_graph(ontology_path: str | Path) -> GraphDocument:
    ...
    if "medical" in name:
        return load_medical_ontology_graph(ontology_path)
...
def load_medical_ontology_graph(ontology_path: str | Path) -> GraphDocument:
    """Structured-output T-Box for one MedicalCase graph."""
    graph = Graph()
    graph.parse(str(ontology_path), format="turtle")
    ...
    for cls in graph.subjects(RDF.type, OWL.Class):
        if not isinstance(cls, URIRef):
            continue
        iri = str(cls)
        if iri.startswith(MEDICAL) and not iri.endswith("/Thing"):
            class_iris.add(iri)
    class_iris.add(f"{MEDICAL}MedicalCase")
    ...
    return GraphDocument(
        nodes=nodes,
        relationships=relationships,
        source=Document(page_content="MedicalCase ontology", metadata={"ontology_uri": MEDICAL}),
    )
