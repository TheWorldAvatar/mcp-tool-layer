...
def _run_mcp_dir(run: str, hash_id: str) -> Path:
    candidate = Path(run)
    if candidate.is_absolute() or any(sep in run for sep in ("/", "\\")):
        if (candidate / "mcp_run").is_dir():
            return candidate / "mcp_run"
        if (candidate / "runtime" / hash_id / "mcp_run").is_dir():
            return candidate / "runtime" / hash_id / "mcp_run"
        if (candidate / hash_id / "mcp_run").is_dir():
            return candidate / hash_id / "mcp_run"
        return candidate / "mcp_run"
    return REPO_ROOT / "scenarios" / "mops" / "runs" / run / "runtime" / hash_id / "mcp_run"


def hint_dir(hash_id: str) -> tuple[Path, str] | tuple[None, None]:
    for run in HINT_RUNS:
        path = _run_mcp_dir(run, hash_id)
        files = [
            item
...
def _with_kg_budget(hash_id: str, entity: HintEntity) -> HintEntity:
    ...
    for run in dict.fromkeys((entity.run, *BUDGET_RUNS)):
        ...
        raise FileNotFoundError(
            "No pipeline KG token traces for "
            f"{hash_id} / {entity.label!r}: " + " | ".join(errors)
        )
    return HintEntity(
        key=entity.key,
        label=entity.label,
...
def distill_extension_hint(prompt_text: str, domain: str = "ontospecies") -> str:
    ...
    heading = "OntoSpecies" if domain == "ontospecies" else "OntoMOPs"
    return (
        "SEMANTIC_HINTS_V1\n"
        f"Whole-graph ledger: {heading} occurrence view of one bound top-level entity.\n\n"
        "=== EXTENSION SEMANTIC_HINTS ===\n"
        f"{ledger}\n"
    )
...
def _hint_prefix_score(stem: str, label: str) -> int:
    ...
    for left, right in zip(candidate, target):
        if left != right:
            break
        score += 1
    # Long truncated stems need >=8 shared chars (NDBDC vs ADBDC).
    # Short product codes such as UMC-1 match only when the prefix is exact.
    return score if candidate == target or score >= 8 else -1
...
def extension_hint_path(
    ...
    names = [artifact]
    if uri:
        scoped = entity_scope_name(label, uri)
        if scoped not in names:
            names.append(scoped)
    for name in names:
        matches = sorted(directory.glob(f"{domain}_iter*_hints_{name}.txt"))
        if matches:
            return matches[-1]
