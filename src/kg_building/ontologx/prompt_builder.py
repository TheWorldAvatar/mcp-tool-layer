...
def generic_graph_rules(
    *,
    bound_root: str = "ontosyn:ChemicalSynthesis",
    prefixes: str = "ontosyn:, om-2:, ontomops:, rdfs:label",
    specific_example: str = "ontosyn:Add instead of ontosyn:SynthesisStep",
) -> str:
    """Original OntoLogX operational constraints, rewritten for one bound root."""
    bound_local = bound_root.split(":")[-1]
    return f"""# Generic OntoLogX graph rules
These are the original OntoLogX operational constraints, rewritten for one bound {bound_local}.

- Emit exactly one {bound_root}: the bound root named in the human message. Do not mint a replacement root.
- Use only types, properties, and relationships allowed by the ontology / SynthesisGraph schema. Do not invent new ones.
- Use the most specific type available for nodes and relationships, e.g. {specific_example}.
- Respect the appropriate casing and prefixes for types and properties ({prefixes}).
...
- The graph must be connected: every emitted node must be reachable from the bound {bound_local} through ontology relationships.
""".strip()


# Shared by official no-prompt and strict no-prompt. No domain construction recipes.
OX_GENERIC_GRAPH_RULES = generic_graph_rules()
...
DEFAULT_MEDICAL_TBOX = (
    REPO_ROOT / "data" / "ontologies" / "medical_case_schema_de_non_flat_v4.ttl"
)


def build_medical_noprompt_prompt(tbox_path: Path | None = None) -> str:
    """OX generic-noprompt medical surface: occurrence is added by strict_noprompt.

    Kept as a compatibility alias for tests. The locked medical noprompt prompt
    is ``build_generic_noprompt_medical_prompt``.
    """
    from strict_noprompt import build_generic_noprompt_medical_prompt

    del tbox_path
    return build_generic_noprompt_medical_prompt()
...
def build_system_prompt(
    ...
    path = tbox_path or resolve_default_tbox()
    tbox = path.read_text(encoding="utf-8").strip()
    if not tbox:
...
def build_full_prompt_system_prompt(tbox_path: Path | None = None) -> str:
    """Historical official OX system: from_extraction + full_hints + ONEPASS + T-Box.

    This is the primary system, not an appendix on the occurrence map.
    """
