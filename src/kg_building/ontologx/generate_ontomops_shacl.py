...
ontomops:UniqueMopPerOutputShape a sh:NodeShape ;
  sh:targetClass ontosyn:ChemicalOutput ;
  sh:sparql [
    sh:message "A ChemicalOutput may be isRepresentedBy at most one MetalOrganicPolyhedron" ;
    sh:select '''
      PREFIX ontosyn: <https://www.theworldavatar.com/kg/OntoSyn/>
      PREFIX ontomops: <https://www.theworldavatar.com/kg/ontomops/>
      SELECT $this
      WHERE {
        $this ontosyn:isRepresentedBy ?a, ?b .
        FILTER(?a != ?b)
        ?a a ontomops:MetalOrganicPolyhedron .
        ?b a ontomops:MetalOrganicPolyhedron .
      }
    '''
  ] .

ontomops:CbuFormulaRequiredShape a sh:NodeShape ;
  sh:targetClass ontomops:ChemicalBuildingUnit ;
  sh:sparql [
    sh:message "ChemicalBuildingUnit must carry hasCBUFormula" ;
    sh:select '''
      PREFIX ontomops: <https://www.theworldavatar.com/kg/ontomops/>
      SELECT $this
      WHERE {
        FILTER NOT EXISTS { $this ontomops:hasCBUFormula ?f . }
      }
    '''
  ] .
"""
