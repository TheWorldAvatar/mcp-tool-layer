"""In-memory generation context shared by compile, authoring, and validation.

`AgenticGenerationContext` is the single object later stages read. The
lower-level builder here assumes a legacy meta-task adapter; the public
CLI path goes through `artifact_compiler.build_domain_generation_context`.
...
DEFAULT_AGENTIC_OUTPUT_ROOT = Path("generated")
...
class OntologySpec:
    """Identity of one ontology in a run: name, TTL path, role, adapter path."""
...
class AgenticGenerationContext:
    """Parsed T-Box, compiled contract, paths, and iteration blueprint for one ontology."""
    ...
    def to_json_dict(self) -> dict[str, Any]:
        """Serialize the context for `generated/ontology_structures/` dumps."""
        return asdict(self)
    ...
    if not candidate.is_absolute():
        ...
        candidates.extend(
            [
                ...
                discover_repository_root() / candidate,
            ]
        )
...
def resolve_default_config_for_ontology(ontology_name: str) -> Path:
    """Resolve `configs/domains/<ontology>.json` from the repository root."""
    from src.extraction_prompt_generation.paths import resolve_domain_config_path

    return resolve_domain_config_path(ontology_name=str(ontology_name or "").strip())
...
def resolve_ontology_spec(
    *,
    ...
    """Resolve name, TTL path, and main/extension role from the adapter JSON."""
...
def build_agentic_generation_context(
    *,
    ...
    """Parse one TTL and compile contracts from a legacy meta-task adapter.

    The CLI does not call this directly. `artifact_compiler` writes that
    adapter from domain config, then uses this builder as the inner step.
    """
    ...
    if not ttl_path.is_file():
        ttl_path = resolve_under_repository(spec.ttl_file)
    if not ttl_path.is_file():
    ...
    if not root.is_absolute():
        ...
        root = resolve_under_repository(root)
...
def build_contexts_for_ontologies(
    ...
    *,
    ...
    """Build one context per ontology from a shared meta-task adapter."""
