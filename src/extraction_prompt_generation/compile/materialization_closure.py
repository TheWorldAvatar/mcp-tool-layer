...
The closure is evidence-driven. An optional source fact does not assert
that an individual must exist; it asserts that, when the fact is found,
the pipeline has a legal way to materialize its target. See compile/README.md.
"""
...
def derive_creator_surface(context: Any) -> list[dict[str, Any]]:
    ...
    for raw in owned_entity_tool_contracts(context):
