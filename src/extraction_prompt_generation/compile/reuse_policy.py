"""Normalize GPT-5 class-reuse judgments and project them onto runtime tools.

Normalize a generated 10/10 policy (`llm-class-reuse-policy.v1`) or an
optional override review (`binary-class-reuse-review.v0`). This module validates that payload,
maps reuse scopes onto storage (central / document / scoped / none), and
derives existing-entity check contracts. It does not call the model.
...
def reuse_storage_scope(reuse_scope: str, *, reusable: bool) -> str:
    """Map class-reuse policy vocabulary to one deterministic runtime store."""
...
ACCEPTED_SOURCE_SCHEMAS = frozenset(
    {
        "llm-class-reuse-policy.v1",
        "binary-class-reuse-review.v0",
    }
)


def normalize_reuse_policy(
    payload: dict[str, Any], *, source_text: str
) -> dict[str, Any]:
    """Normalize a pinned or generated class-reuse policy for the contract."""
    if not isinstance(payload, dict):
        raise ValueError("reuse policy must be a JSON object")
    schema = str(payload.get("schema_version") or "").strip()
    if schema not in ACCEPTED_SOURCE_SCHEMAS:
        raise ValueError(
            "reuse policy must use llm-class-reuse-policy.v1 or "
            "binary-class-reuse-review.v0"
        )
    ...
    generated_schema = schema == "llm-class-reuse-policy.v1"
    for index, item in enumerate(payload.get("classes") or []):
        ...
        if generated_schema and not reusable and not reuse_scope:
            reuse_scope = "occurrence_local"
        if generated_schema and not reusable and not match_basis:
...
def load_reuse_policy(path: str | Path) -> dict[str, Any]:
    """Load and normalize a generated operational class-reuse policy."""
    policy_path = Path(path).resolve()
    raw_text = policy_path.read_text(encoding="utf-8")
    payload = json.loads(raw_text)
    if not isinstance(payload, dict):
        raise ValueError("reuse policy must be a JSON object")
    return normalize_reuse_policy(payload, source_text=raw_text)


def attach_reuse_policy(
    contract: dict[str, Any], policy_path: str | Path
) -> dict[str, Any]:
    """Attach a validated generated policy to a generation contract."""
