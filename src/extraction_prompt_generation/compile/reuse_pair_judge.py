"""Runtime pair-identity judge for generated RDF reuse, not prompt compile.

Copied into generated packages when central reuse needs an LLM grant.
Prompt generation uses `reuse_judgment.py` instead. See compile/README.md.
"""
...
class ReuseJudgeConfig:
    ...
    def from_environment(cls) -> "ReuseJudgeConfig":
        ...
        repo = discover_repository_root()
        cache_raw = str(os.environ.get("TWA_REUSE_JUDGE_CACHE_DIR") or "").strip()
        cache = (
            resolve_under_repository(cache_raw)
            if cache_raw
            else repo / "tmp" / "entity_reuse_judge"
        )
        audit_raw = str(os.environ.get("TWA_REUSE_JUDGE_AUDIT_DIR") or "").strip()
        audit = (
            resolve_under_repository(audit_raw)
            if audit_raw
            else repo / "tmp" / "reuse_judge_audit"
        )
        return cls(
