...
and no ontology-specific prefixes. Domain JSON must not contain a SPARQL string.
...
def validate_enrichment_target_declaration(raw: Any, *, prefix: str) -> dict[str, Any]:
    ...
    root_variable = str(raw.get("root_variable") or "root").strip() or "root"
...
def resolve_enrichment_target_sparql_path(
    ...
    *,
    ...
    candidates: list[Path] = [resolve_generated_package_root() / relative]
    override = (
    ...
    if override:
        ...
        candidates.append(
            override_path / relative
            if override_path.is_absolute()
            else root / override / relative
        )
    candidates.append(root / "generated" / relative)
