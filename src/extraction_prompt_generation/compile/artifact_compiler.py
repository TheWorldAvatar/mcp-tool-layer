"""Build the domain generation context from T-Box + one human domain config.

This is the compile-stage orchestrator: parse the T-Box bundle, run GPT-5
top-class / extension-focus planning, generate a first-valid class-reuse
policy from T-Box + compiled plan, compile the iteration plan, and attach
materialization contracts. Prompt text is not authored here.

See compile/README.md for the file map.
"""
...
from src.extraction_prompt_generation.config.domain_config import (
    ...
    LOCKED_EXTENSION_HINT_REPRESENTATION,
...
from src.extraction_prompt_generation.compile.extension_bridge import (
...
INHERITED_MAIN_ENTITY_SOURCE = "mcp_run/iter1_top_entities.json"


def _domain_role(config: DomainGenerationConfig) -> str:
    """Return `main` or `extension` from runtime.binding.role."""
    return str((config.runtime.get("binding") or {}).get("role") or "main").strip()


def _load_upstream_planned_top_entity(
    *,
    output_root: Path,
    upstream_ontology: str,
    upstream_tbox: Path,
    repository_root: Path,
) -> tuple[dict[str, Any], Path]:
    """Read the parent domain's GPT-5 top entity; never take it from this config."""
    from src.extraction_prompt_generation.paths import resolve_domain_config_path

    parent_config = load_domain_generation_config(
        resolve_domain_config_path(ontology_name=upstream_ontology),
        repository_root=repository_root,
    )
    if parent_config.role != "main":
        raise ValueError(
            f"runtime.binding.upstream_ontology={upstream_ontology!r} must be a "
            "main domain, not another extension"
        )
    if parent_config.primary_tbox != upstream_tbox:
        raise ValueError(
            "runtime.binding.upstream_tbox must match "
            f"{upstream_ontology} tbox.primary ({parent_config.primary_tbox})"
        )
    candidates = (
        output_root
        / "ontology_structures"
    ...
    for path in candidates:
        ...
        if not isinstance(raw, dict):
            continue
        planned = dict(raw.get("top_entity") or {})
        if not str(planned.get("class_local") or "").strip():
            continue
        payload = raw
        source_path = path
        break
    if payload is None or source_path is None:
        raise FileNotFoundError(
            f"extension binding needs the GPT-5 top entity from {upstream_ontology}. "
            "Generate that parent into the same --output-root first, e.g. "
            f"python -m src.extraction_prompt_generation {upstream_ontology} "
            "--stage context"
        )
    top = dict(payload.get("top_entity") or {})
    if top.get("owned_by_extension") is False:
        raise ValueError(
            f"upstream {upstream_ontology} top entity is itself inherited; "
            "extensions must bind to a main ontology"
        )
    ...
    upstream_class = (upstream_parsed.get("classes") or {}).get(class_local) or {}
    expected_iri = str(upstream_class.get("iri") or "").strip()
    if not class_local or not expected_iri:
        raise ValueError(
            f"upstream {upstream_ontology} planned top entity {class_local!r} "
            "is absent from upstream_tbox"
        )
    if class_iri and class_iri != expected_iri:
        raise ValueError(
            f"upstream {upstream_ontology} planned top entity IRI "
            f"{class_iri} does not match upstream_tbox {expected_iri}"
        )
    return (
        {
            "status": "known",
            "class_local": class_local,
            "class_iri": expected_iri,
            "rationale": str(top.get("rationale") or "").strip()
            or "Inherited from the upstream GPT-5 top-entity decision.",
            "evidence": list(top.get("evidence") or [class_local]),
            "source": "upstream_generated_top_entity",
            "inherited_from_ontology": upstream_ontology,
            "entity_source": INHERITED_MAIN_ENTITY_SOURCE,
            "owned_by_extension": False,
            "main_pass_reuses_scoped_root": True,
            "upstream_artifact": str(source_path),
        },
        source_path,
    )
...
def _runtime_blueprint(
    ...
    runtime_model = config.models.get("runtime_extraction") or LOCKED_EXTRACTION_MODEL
    ...
    for index, (semantic, runtime, profile_slot) in enumerate(
        ...
        if config.role == "extension" and str(
            iteration.get("hint_representation") or ""
        ).strip() != LOCKED_EXTENSION_HINT_REPRESENTATION:
            raise ValueError(
                "extension extraction is locked to hint_representation="
                f"{LOCKED_EXTENSION_HINT_REPRESENTATION!r}"
            )
...
def _legacy_adapter(
    ...
    *,
    ...
    if role == "main":
        ...
        for index, raw_extension in enumerate(config.runtime.get("extensions") or []):
            ...
            if not extension_path.is_absolute():
                extension_path = (config.repository_root / extension_path).resolve()
    ...
    if not upstream_path.is_absolute():
        upstream_path = (config.repository_root / upstream_path).resolve()
...
def build_domain_generation_context(
    *,
    ...
    if operation_mode != "legacy":
        raise ValueError(
            "This package only supports operation_mode='legacy' (GPT-5 thick prompts)"
        )
    ...
    role = _domain_role(config)
    planning_model_key = (
        "extension_focus_planning" if role == "extension" else "top_entity_planning"
    )
    planning_model = str(config.models.get(planning_model_key) or "").strip()
    reuse_judgment_model = str(config.models.get("reuse_judgment") or "").strip()
    if not planning_model:
        raise ValueError(f"domain config models.{planning_model_key} is missing")
    if config.reuse_policy_path is None and not reuse_judgment_model:
        raise ValueError("domain config models.reuse_judgment is missing")
    ...
    if role == "extension":
        ...
        if not upstream_tbox.is_absolute():
            upstream_tbox = (repo / upstream_tbox).resolve()
        inherited_top, inherited_source = _load_upstream_planned_top_entity(
            output_root=root,
            upstream_ontology=upstream_name,
            upstream_tbox=upstream_tbox,
        ...
        decisions["inherited_top_entity_artifact"] = str(inherited_source)
    ...
    plan_provenance = {
        ...
        "ownership_sha256": str(
            (decisions.get("assignments") or {}).get("ownership_sha256") or ""
        ),
    }
    plan = compile_iteration_plan(
        ...
        contract=base.contract,
        ontology_name=config.ontology_name,
        blueprint_provenance=plan_provenance,
    )
    reuse_policy_path = derived / "reuse_policy.json"
    pinned_reuse_path = config.reuse_policy_path
    if pinned_reuse_path is not None:
        reuse_policy_path.write_text(
            pinned_reuse_path.read_text(encoding="utf-8"),
            encoding="utf-8",
        )
        reuse_cached = False
        reuse_policy_source = "optional_reuse_policy_override"
        reuse_policy_model = ""
        reuse_policy_origin = str(pinned_reuse_path)
    else:
        generated_reuse = generate_reuse_policy(
            primary_tbox=config.primary_tbox,
            supporting_tboxes=config.supporting_tboxes,
            compiled_plan=plan,
            domain_config_path=config.path,
            output_path=reuse_policy_path,
            model=reuse_judgment_model,
            top_entity=decisions.get("top_entity"),
            repository_root=repo,
        )
        reuse_cached = bool(generated_reuse.pop("_cached", False))
        reuse_policy_source = "tbox_reuse_first_valid"
        reuse_policy_model = reuse_judgment_model
        reuse_policy_origin = str(reuse_policy_path)
    reuse_policy = attach_reuse_policy(base.contract, reuse_policy_path)
    plan = compile_iteration_plan(
        blueprint=runtime_blueprint,
        parsed=base.parsed,
        contract=base.contract,
    ...
    reuse_policy = attach_reuse_policy(context.contract, reuse_policy_path)
    if role == "extension":
    ...
    context = replace(
        ...
        config_provenance={
            **context.config_provenance,
            ...
            "semantic_decisions": {
                "source": "planned_top_entity_plus_deterministic_ownership",
                "model": planning_model,
                "model_key": planning_model_key,
                ...
                "inherited_top_entity_source": (
                    str(decisions.get("inherited_top_entity_artifact") or "")
                    if role == "extension"
                    else ""
                ),
            },
            "reuse_policy": {
                "source": reuse_policy_source,
                "model": reuse_policy_model,
                "path": reuse_policy_origin,
            ...
            "models": {
                ...
                **(
                    {"reuse_judgment": reuse_judgment_model}
                    if reuse_policy_source == "tbox_reuse_first_valid"
                    else {}
                ),
            },
            "boundary": {
                ...
                "reuse_policy": reuse_policy_source,
    ...
    manifest = {
        ...
        "reuse_policy_sha256": reuse_policy["source_sha256"],
        "reuse_policy_cached": reuse_cached,
        "planning_model": planning_model,
        "planning_model_key": planning_model_key,
        "reuse_judgment_model": reuse_judgment_model,
