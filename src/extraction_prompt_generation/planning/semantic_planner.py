"""Top-class / extension-focus planning, then deterministic ownership.

The model (default `gpt-5`) is allowed two decisions: which class is the
pipeline root (or extension focus), and nothing else. Slot ownership is
computed by `iteration_assigner`. Attempt audits land under
...
from src.extraction_prompt_generation.planning.iteration_assigner import (
...
def plan_top_entity_semantics(
    *,
    ...
    planning_model = str(model or PLANNING_MODEL).strip() or PLANNING_MODEL
    ...
    for attempt in range(1, MAX_SEMANTIC_ATTEMPTS + 1):
        ...
        if top_entity is not None:
            top_entity["model"] = planning_model
        _write_attempt(
            audit_dir,
            phase=phase,
            attempt=attempt,
...
def plan_domain_semantics(
    *,
    ...
    """Use the configured model for the two authorized domain-semantic planning decisions."""
    ...
    planning_key = (
        ...
        else "top_entity_planning"
    )
    planning_model = str(config.models.get(planning_key) or PLANNING_MODEL).strip() or PLANNING_MODEL
    projection = _ontology_projection(parsed)
    classes = projection["classes"]
    properties = projection["properties"]
    selected_planner = (
