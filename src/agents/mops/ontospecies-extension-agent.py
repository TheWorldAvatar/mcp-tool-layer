...
async def species_extension_agent():
    ...
    response, metadata = await agent.run(PROMPT.format(ontosynthesis_a_box=TEST_ONTOSYNTHESIS_A_BOX), recursion_limit=500)
    return response
...
async def species_extension_agent_with_content(paper_content):
    ...
    mcp_tools = ["ontospecies_extension"]
    agent = BaseAgent(model_name="gpt-4o-mini", model_config=model_config, remote_model=True, mcp_tools=mcp_tools, mcp_set_name="extension.json")
    response, metadata = await agent.run(PROMPT.format(ontosynthesis_a_box=TEST_ONTOSYNTHESIS_A_BOX, paper_content=paper_content), recursion_limit=500)
    return response
