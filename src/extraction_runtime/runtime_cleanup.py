"""Fresh-start runtime cleanup and conversion-artifact seeding."""
...
_CONVERSION_ARTIFACT_SUFFIXES = (".md", "_text.md", "_tables.md", "_vision.md")
...
def _assert_safe_runtime_path(data_dir: str | Path, repository_root: Path) -> Path:
    ...
    try:
        ...
        raise ValueError(
            f"Fresh-start cleanup requires data_dir inside the repository: {runtime}"
        ) from exc
...
def _seed_conversion_artifacts(
    *,
    ...
    requested = {
        str(value).strip() for value in (selected_hashes or []) if str(value).strip()
    }
    copied: list[str] = []
