"""Locked ref-entity-relations.v1 shape checks from the official runtime."""
...
def parse_json_payload(text: str) -> Any:
    try:
        ...
        raise ValueError(
            f"Invalid JSON extraction payload: {exc}; payload preview={preview}"
        ) from exc
    ...
    if expected_schema == "ref-entity-relations.v1" or is_ref_entity_relations:
        ...
        for index, entity in enumerate(entities):
            ...
            if not isinstance(datatype_properties, dict):
                errors.append(f"entities[{index}].datatype_properties must be an object")
        ...
        if prior_text:
            ...
            if not isinstance(prior_payload, dict):
                errors.append("accumulated ref-entity-relations hints must be a JSON object")
            else:
