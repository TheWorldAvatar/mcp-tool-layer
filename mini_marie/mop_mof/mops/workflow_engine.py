"""
MOPs TWA workflow engine: online probe (limited SPARQL) -> record -> offline replay.

Supports chained steps, $variables, transform steps, tool steps, and local SPARQL plans.
"""
...
from mini_marie.mop_mof.mops import twa_operations as ops
from mini_marie.mop_mof.mops.sparql_plans import (
    DEFAULT_OFFLINE_CAP,
...
def _stats_rows() -> List[Dict[str, Any]]:
    stats = ops.get_twa_statistics()
    return [{"metric": k, "value": str(v)} for k, v in stats.items()]


TOOL_REGISTRY: Dict[str, Callable[..., List[Dict[str, Any]]]] = {
    "lookup_synthesis": lambda **kw: ops.lookup_synthesis_iri(kw["name"]),
    "get_synthesis_recipe": lambda **kw: ops.get_synthesis_recipe(kw["synthesis_name"]),
    "get_synthesis_steps": lambda **kw: ops.get_synthesis_steps(kw["synthesis_name"]),
    "get_mop_building_units": lambda **kw: ops.get_mop_building_units(kw["mop_name"]),
    "get_twa_statistics": lambda **_: _stats_rows(),
}
...
def run_transform_step(step_index: int, spec: Dict[str, Any], variables: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if transform == "top_n_by_field":
        ...
        out_field = spec.get("output_field", "mopLabel")
        if out_field == "*":
            output = top_rows
        else:
...
def run_workflow(
    ...
    if workflow.get("synthesis_name") is not None:
        variables["synthesis_name"] = workflow.get("synthesis_name")
    if workflow.get("mop_name") is not None:
        variables["mop_name"] = workflow.get("mop_name")
    ...
    result: Dict[str, Any] = {
        ...
        "synthesis_name": workflow.get("synthesis_name"),
        "mode": mode,
