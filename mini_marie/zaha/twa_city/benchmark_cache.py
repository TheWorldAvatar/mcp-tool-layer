"""
Compare cold (SPARQL fetch) vs warm (SQLite cache hit) workflow performance.
"""
...
REPORT_PATH = Path(__file__).resolve().parent / "workflow_runs" / "cache_benchmark.json"
...
def benchmark_workflow(
    ...
    *,
    ...
    sparql_steps = sum(1 for s in warm.get("call_trace", []) if s.get("step_type") == "sparql_plan")

    print(
        f"  cold={cold_wall}ms warm={warm_wall}ms speedup={speedup}x "
        f"cache_hits={cache_hits}/{sparql_steps}"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="City cache cold vs warm benchmark")
    parser.add_argument(
        "--workflow",
        default="top10_buildings_locations_kl",
        help="Workflow stem (default: KL top10 — faster than Bremen full list)",
    )
    parser.add_argument("--modes", default="online,offline", help="Comma-separated: online,offline")
